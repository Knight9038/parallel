...
void luDecomposition(vector<vector<float>>& mat, vector<vector<float>>& lower, vector<vector<float>>& upper, int rank, int size) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        MPI_Bcast(&upper[i][0], n, MPI_FLOAT, i / rows_per_proc, MPI_COMM_WORLD);

        for (int j = max(i + 1, start_row); j < end_row; j++) {
            float sum = 0.0;
            for (int k = 0; k < i; k++) {
...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        cout << "Parallel (MPI with row partitioning): " << count << "ms" << endl;
    }
